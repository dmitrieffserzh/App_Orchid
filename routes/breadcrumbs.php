...
Breadcrumbs::for('platform.pages', function ($trail) {
    $trail->parent('platform.index');
    $trail->push(__('Страницы'), route('platform.pages'));
});
// Platform -> Pages -> Create
Breadcrumbs::for('platform.pages.create', function ($trail) {
    $trail->parent('platform.pages');
    $trail->push(__('Создать страницу'), route('platform.pages.create'));
});
// Platform -> Pages -> Edit
Breadcrumbs::for('platform.pages.edit', function ($trail, $page) {
    $trail->parent('platform.pages');
    $trail->push(__('Редактировать страницу'), route('platform.pages.edit', $page));
});

// Platform -> Settings
Breadcrumbs::for('platform.settings', function ($trail) {
    $trail->parent('platform.index');
    $trail->push(__('Параметры системы'));
});
